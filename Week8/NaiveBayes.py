...
def train_nb(X, y):
    ...
    for i in range(n):
        xi = X.iloc[:, i]
        p_xi = namedtuple(
            X.columns[i], ["is_continuous", "conditional_pro"]
        )  # 用于储存每个变量的情况
        ...
        if is_continuous:  # 连续值时，conditional_pro 储存的就是 [mean, var] 即均值和方差
            ...
            xi1_value_count = (
                pd.value_counts(xi1).reindex(unique_value).fillna(0) + 1
...
if __name__ == "__main__":
    data_path = r"E:\code\python\ML_WeekWork\watermelon3_0_Ch.csv"
